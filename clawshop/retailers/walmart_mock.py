"""Walmart Tier B adapter. PRD §13.1 / §13.2 — deep-link + prefilled cart."""
...
class WalmartMockAdapter(RetailerAdapter):
    name = "walmart"
    ...
    async def build_cart(self, items: list[LineItem]) -> Cart:
        ...
        return Cart(
            retailer=self.name,
            items=items,
            deep_link=f"https://walmart.example/cart?items={skus}",
            total=total,
        )
    ...
    async def get_order(self, order_id: str) -> Order:
        raise NotImplementedError("mock: orders are not persisted in this prototype")
